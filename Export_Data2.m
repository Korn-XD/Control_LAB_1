...
try
    if exist('out', 'var')
        ...
        sig_speed_sim = out.sim_speed_sim;
    else
        % กรณีไม่ได้ติ๊ก Single simulation output
        sig_volt = sim_volt;
        sig_speed = sim_speed;
        sig_speed_sim = sim_speed_sim;
    end
...
nexttile;
plot(sig_speed_sim.Time, squeeze(sig_speed_sim.Data), 'B', 'LineWidth', 1.5); % สีม่วง Magenta (ชัดบนดำ)
title(['Motor Speed Simulate: ' my_mode_name], 'Color', 'w');
ylabel('Speed (rad/s)', 'Color', 'w');
xlabel('Time (s)', 'Color', 'w');
grid on;
set(gca, 'Color', 'k', 'XColor', 'w', 'YColor', 'w', 'GridColor', 'w', 'GridAlpha', 0.3);
...
save(mat_filename, 'sig_volt', 'sig_speed','sig_speed_sim', 'my_mode_name', 'timestamp');
